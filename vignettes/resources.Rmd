...
Each of those files is represented by a Uniform Resource Identifier (URI) in the metadata tables returned by the functions supplied to the `FUN.datasets` and `FUN.initial` arguments of the `iSEEindex()` function.

`r Biocpkg("iSEEindex")` requires each URI to contain a scheme ([Wikipedia][scheme-wikipedia] `r fontawesome::fa("external-link")`) component, to identify the type of resource and invoke the associated method for accessing it.
...
The method is invoked to process the raw value of the URI, download the resource file locally using custom code if necessary, cache the file using `r Biocpkg("BiocFileCache")`, and return the path to the cached file.
