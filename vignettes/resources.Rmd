...
Each of those files is represented by a Uniform Resource Identifier (URI) in the metadata tables returned by the functions supplied to the `FUN.datasets` and `FUN.initial` arguments of `iSEEindex()`.
...
On first use, `precache()` processes the raw value of the URI, download the resource file locally using custom code if necessary, cache the file using `r Biocpkg("BiocFileCache")`, and return the path to the cached file.
